...
const Home: NextPage = ({ images }: { images: ImageProps2[] }) => {
  const arrayOfficielDesinger: string[] = [
    "Prinker Korea Inc.",
    "PrinkerAz",
    "Pixabay",
    "M_Ink",
  ...
  const reorderedImgList = jsonDataLiked.contents
    .map((f, ik) => {
      return { id: ik, ...f };
    })
    .filter((f) => arrayOfficielDesinger.includes(f.nickname))
      ...
      <main className="mx-auto max-w-[1960px] p-4">
        ...
        <div className="columns-1 gap-4 sm:columns-2 xl:columns-3 2xl:columns-4">
          <div className="after:content relative mb-5 flex h-[629px] flex-col items-center justify-end gap-4 overflow-hidden rounded-lg bg-white/10 px-6 pb-16 pt-64 text-center text-white shadow-highlight after:pointer-events-none after:absolute after:inset-0 after:rounded-lg after:shadow-highlight lg:pt-0">
            ...
            <h1 className="mb-4 mt-8 text-6xl font-bold uppercase tracking-widest ">
              {" "}
              QUINK
            </h1>
            ...
            <input
              className="z-50 h-14 w-96 rounded-md bg-gray-200 px-2 py-2 text-lg text-gray-800 "
              placeholder="search"
              type="text"
              onChange={(e) => console.log(e)}
            />
            <p className="max-w-[40ch] text-white/75 sm:max-w-[32ch]">
              Lorem ipsum dolor sit consectetur adipisicing elit. Id esse error
              minima expedita exercitationem maiores.
            </p>
            <button
          ...
          {reorderedImgList.map((image: ImageProps2) => (
            ...
            >
              ...
              <p className="text-xs text-white">
                {image.width + " X " + image.height + "___" + image.workWidth}
              </p>
            </Link>
          ))}
        </div>
